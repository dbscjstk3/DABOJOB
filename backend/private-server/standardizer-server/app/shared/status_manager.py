"""
공통 상태 관리자 - Standardizer Server용
"""
import json
import logging
import asyncio
...
class SharedStatusManager:
    ...
    async def get_dashboard_stats(self) -> Dict[str, Any]:
        """대시보드용 통계 조회"""
        if not self.redis:
            return {"total": 0, "by_status": {}, "by_stage": {}, "bottlenecks": []}
        ...
        try:
            ...
            jobs = []
            for key in keys[:1000]:  # 최대 1000개로 제한
                data = await self.redis.hget(key, "data")
                if data:
            ...
            stats = {
                ...
                "by_status": {},
                "by_stage": {
                    "mapping": {"pending": 0, "processing": 0, "suggested": 0, "verified": 0, "failed": 0},
                    "summary": {"processing": 0, "completed": 0, "failed": 0},
                    "news": {"processing": 0, "completed": 0, "finished": 0, "failed": 0}
                },
                "bottlenecks": []
            }

            for job in jobs:
                status = job.get("current_status", "unknown")
                ...
                for stage_name, stage_data in stages.items():
                    ...
                    if stage_name in stats["by_stage"]:
                        for key in stats["by_stage"][stage_name]:
                            if key in stage_status:
                                stats["by_stage"][stage_name][key] += 1
                                break

            # 병목 감지
            suggested_count = stats["by_stage"]["mapping"]["suggested"]
            if suggested_count > 10:
                stats["bottlenecks"].append({
                    "stage": "mapping",
                    "issue": f"{suggested_count}개 기업이 수동 검증 대기 중",
