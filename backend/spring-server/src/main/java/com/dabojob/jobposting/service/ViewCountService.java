...
public class ViewCountService {
    ...
    public void incrementViewCount(String jobPostingId, String userId) {
        try {
            ...
            // 조회수 실패는 핵심 기능에 영향을 주지 않으므로 예외를 던지지 않음
        }
    }
    ...
    public Set<String> getHotJobPostings(int limit) {
        try {
            ...
            Set<String> hotJobs = stringRedisTemplate.opsForZSet().reverseRange(HOT_JOBS_KEY, 0, limit - 1);
            return hotJobs != null ? hotJobs : Collections.emptySet();
    ...
    public boolean hasUserViewedRecently(String jobPostingId, String userId) {
        try {
            ...
            // 조회 실패 시 false 반환 (중복 체크 실패시 조회수 증가 허용)
            return false;
        }
    }
